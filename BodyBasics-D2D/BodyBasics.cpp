...
int OpenComPort()
{
	hFile = CreateFile(L"\\\\.\\COM13", GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
	...
	if (GetCommState(hFile, &port)
		//&& BuildCommDCB(L"baud=115200 parity=N data=8 stop=1 dtr=off rts=off", &port)
		&& BuildCommDCB(L"baud=115200 parity=N data=8 stop=1", &port)
		&& SetCommState(hFile, &port))
	{
		//DWORD written;
		//const char *str = "well now, let's see if this worked...2\n";
		//WriteFile(hFile, str, strlen(str), &written, NULL);
		return 1;
	}

	//std::cout << "can't configure com port\n";
	return 0;
}

//
int sendStrToRemote(const char *str)
{
	DWORD written;
	WriteFile(hFile, str, strlen(str), &written, NULL);
	return 1;
...
//
int sendReportToRemote(GAMEPAD &gamepad)
{
	if (memcmp(&gamepad, &prevGamepad, sizeof(GAMEPAD)) == 0)
		return 0;
	...
	DWORD written;
	WriteFile(hFile, code, 8, &written, NULL);
	prevGamepad = gamepad;
	return 1;
}
...
int CBodyBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	WCHAR str[64];
	StringCchPrintf(str, _countof(str), L"%d", 0);
	if (m_hWnd)
		SetDlgItemText(m_hWnd, IDC_STATUS2, str);
...
#define POS_THRESHOLD 0.1
							...
							else if (joints[JointType_HandRight].Position.Y < (joints[JointType_ShoulderRight].Position.Y - POS_THRESHOLD))
							{
								result = L"right";
								gamepad.x1 = 127;
							}
							else if (joints[JointType_HandLeft].Position.Y < (joints[JointType_ShoulderLeft].Position.Y - POS_THRESHOLD))
							{
								result = L"left";
								gamepad.x1 = -127;
							}
		...
		if (m_hWnd)
			SetDlgItemText(m_hWnd, IDC_STATUS2, result);

		//static INT64 lastTx = 0;
		//if (now - lastTx > 50)
		{
			sendReportToRemote(gamepad);
			//lastTx = now;
		}
    }
}
